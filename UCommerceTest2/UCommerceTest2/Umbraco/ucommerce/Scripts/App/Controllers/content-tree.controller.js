function uc_treeController($scope, $rootScope, $window, $location, $q, uCommerceContentService) {
	...
	$scope.shouldHideNodeInTree = function (node) {
		if (node)
	...
	function treeSetSelected(node) {
		if ($scope.multiSelect && $scope.pickertype) {
			if ($scope.multiSelect == "false" && $scope.pickertype.toLowerCase() != node.nodeType.toLowerCase()) {
				return;
			}
		}
