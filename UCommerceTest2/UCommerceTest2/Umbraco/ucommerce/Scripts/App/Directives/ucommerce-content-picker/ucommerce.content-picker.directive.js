		...
		template:
		        '<div class="imagePicker {{contentPickerType}}">' +
				    ...
				    '<div class="imagePickerContainer propertyPane">' +
                        ...
                        '</div>' +
				    '</div>' +
				    '<div class="footerOkCancel" style="margin-top: 15px; margin-left: 20px;"><input data-ng-click="SavePicture($event)" type="button" class="dialog-saveButton" value="{{saveText}}" /><em style="margin: 10px;">{{orText}}</em><a data-ng-click="closeContainer($event)" Class="dialog-cancelButton">{{cancelText}}</a></div>' +
